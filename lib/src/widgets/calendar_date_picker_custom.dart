...
import 'package:calendar_date_picker_custom/src/utils/utils.dart';
...
class CalendarDatePickerCustom extends StatefulWidget {
  CalendarDatePickerCustom({
    ...
    if (config.calendarType == CalendarDatePickerCustomType.single) {
      assert(initialValue.length < 2,
      'Error: single date picker only allows maximum one initial date');
    }

    if (config.calendarType == CalendarDatePickerCustomType.range &&
  ...
  final CalendarDatePickerCustomConfig config;

  @override
  State<CalendarDatePickerCustom> createState() => _CalendarDatePickerCustomState();
}
...
class _CalendarDatePickerCustomState extends State<CalendarDatePickerCustom> {
  ...
  @override
  void didUpdateWidget(CalendarDatePickerCustom oldWidget) {
  ...
  void _handleDayChanged(DateTime value) {
    ...
    setState(() {
      ...
      if (widget.config.calendarType == CalendarDatePickerCustomType.single) {
        selectedDates = [value];
      } else if (widget.config.calendarType == CalendarDatePickerCustomType.multi) {
        final index =
      ...
      } else if (widget.config.calendarType == CalendarDatePickerCustomType.range) {
      ...
      final isValueDifferent =
          widget.config.calendarType != CalendarDatePickerCustomType.single ||
  ...
  Widget build(BuildContext context) {
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        if (widget.config.calendarType == CalendarDatePickerCustomType.range) {
...
/// [CalendarDatePickerCustom], or [showDatePicker2] which create full date pickers.
///
/// See also:
///
///  * [CalendarDatePickerCustom], which provides a Material Design date picker
///    interface.
///
